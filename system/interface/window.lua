  ...
  for _, element in ipairs(table.template) do
    ...
    elseif element.type == 'checkbox' then
      ...
      tmp:SetEventListener('OnValueChanged', function(this, event, checked)
        bxhnz7tp5bge7wvu.settings.store(table.key .. '_' .. element.key, checked)
      end)

      tmp.checkBoxStyle = checkBoxStyle

      tmp:SetChecked(bxhnz7tp5bge7wvu.settings.fetch(table.key .. '_' .. element.key, element.default or false))
      tmp:SetText(element.text)
    ...
    elseif element.type == 'spinner' then
      ...
      tmp_spin:SetNumber(
        bxhnz7tp5bge7wvu.settings.fetch(table.key .. '_' .. element.key, element.default)
      )
      ...
      tmp_spin:SetEventListener('OnValueChanged', function(this, event, userInput, number)
        ...
        bxhnz7tp5bge7wvu.settings.store(table.key .. '_' .. element.key, tonumber(number))
      end)
    ...
    elseif element.type == 'checkspin' then
      ...
      tmp_spin:SetNumber(
        bxhnz7tp5bge7wvu.settings.fetch(table.key .. '_' .. element.key .. '.spin', element.default_spin or 0)
      )
      if tmp_spin.SetStylesheet then tmp_spin:SetStylesheet(spinnerStyleSheet) end
      tmp_spin:ApplySettings()
      ...
      tmp_spin:SetEventListener('OnValueChanged', function(this, event, userInput, number)
        if not userInput then return end
        bxhnz7tp5bge7wvu.settings.store(table.key .. '_' .. element.key .. '.spin', tonumber(number))
      end)
      ...
      tmp_check:SetEventListener('OnValueChanged', function(this, event, checked)
        bxhnz7tp5bge7wvu.settings.store(table.key .. '_' .. element.key .. '.check', checked)
      end)

      tmp_check:SetChecked(bxhnz7tp5bge7wvu.settings.fetch(table.key .. '_' .. element.key .. '.check', element.default_check or false))
    ...
    elseif element.type == 'combo' or element.type == 'dropdown' then
      ...
      tmp_list:SetEventListener('OnValueChanged', function(this, event, value)
        bxhnz7tp5bge7wvu.settings.store(table.key .. '_' .. element.key, value)
      end)

      tmp_list:SetValue(bxhnz7tp5bge7wvu.settings.fetch(table.key .. '_' .. element.key, element.default))
    ...
    elseif element.type == "input" then
      ...
      tmp_input:SetText(bxhnz7tp5bge7wvu.settings.fetch(table.key .. '_' .. element.key, element.default or ''))

      tmp_input:SetEventListener('OnEditFocusLost', function(this)
        bxhnz7tp5bge7wvu.settings.store(table.key .. '_' .. element.key, this:GetText())
      end)
...
function builder.buildGUI(template)
  ...
  if template.profiles == true and template.key_orig then
    ...
    createButton:SetEventListener('OnClick', function()
      ...
      profileButton:SetEventListener('OnClick', function()
        ...
        local profiles = bxhnz7tp5bge7wvu.settings.fetch(template.key_orig .. '_' .. 'profiles', {{key='default',text='Default'}})
        ...
        if profileName ~= '' then
          ...
          bxhnz7tp5bge7wvu.settings.store(template.key_orig .. '_' .. 'profiles', profiles)
          bxhnz7tp5bge7wvu.settings.store(template.key_orig .. '_' .. 'profile', pkey)
          newWindow:Hide()
          parent:Hide()
          parent:Release()
    ...
    deleteButton:SetEventListener('OnClick', function()
      local selectedProfile = bxhnz7tp5bge7wvu.settings.fetch(template.key_orig .. '_' .. 'profile', 'Default Profile')
      local profiles = bxhnz7tp5bge7wvu.settings.fetch(template.key_orig .. '_' .. 'profiles', {{key='default',text='Default'}})
      if selectedProfile ~= 'default' then
        for i,p in ipairs(profiles) do
          if p.key == selectedProfile then
            profiles[i] = nil
            bxhnz7tp5bge7wvu.settings.store(template.key_orig .. '_' .. 'profiles', profiles)
            bxhnz7tp5bge7wvu.settings.store(template.key_orig .. '_' .. 'profile', 'default')
            parent:Hide()
            parent:Release()
            builder.buildGUI(template)
          end
    ...
    local profiles = bxhnz7tp5bge7wvu.settings.fetch(template.key_orig .. '_' .. 'profiles', {{key='default',text='Default'}})
    local selectedProfile = bxhnz7tp5bge7wvu.settings.fetch(template.key_orig .. '_' .. 'profile', 'default')
    ...
    --     bxhnz7tp5bge7wvu.settings.store(template.key_orig .. '_' .. 'profile', value)
    --     parent:Hide()
    --     parent:Release()
    --     builder.buildGUI(template)
    --   end
    ...
    profile_dropdown:SetValue(bxhnz7tp5bge7wvu.settings.fetch(template.key_orig .. '_' .. 'profile', 'Default Profile'))
    --function whChangedProfile(thisprofile)
    --  profile_dropdown:SetValue(bxhnz7tp5bge7wvu.settings.fetch(template.key_orig . .. '_' .. '.' .. thisprofile))
    ...
    --  end
    --    --bxhnz7tp5bge7wvu.settings.store(template.key_orig .. '_' .. 'profile', thisprofile)
    --  parent:Hide()
    --  parent:Release()
    --  builder.buildGUI(template)
    ...
    bxhnz7tp5bge7wvu.settings.store(template.key_orig .. '_' .. 'profile', false)
  end


  if template.key_orig then
    parent:SetEventListener('OnDragStop', function(self, event, left, top)
      bxhnz7tp5bge7wvu.settings.store(template.key_orig .. '_' .. 'window', {left, top})
    end)
    local left, top = unpack(bxhnz7tp5bge7wvu.settings.fetch(template.key_orig .. '_' .. 'window', {false, false}))
  ...
  if not template.color then template.color = bxhnz7tp5bge7wvu.color end
...
bxhnz7tp5bge7wvu.interface.builder = builder

bxhnz7tp5bge7wvu.on_ready(function()
  local engine = {
    key = "_engine",
    title = 'DarkEngine',
    subtitle = 'Build ' .. bxhnz7tp5bge7wvu.version,
    color = '1F8FB5',
    profiles = false,
    width = 250,
    height = 300,
  ...
  bxhnz7tp5bge7wvu.econf = configWindow
end)
