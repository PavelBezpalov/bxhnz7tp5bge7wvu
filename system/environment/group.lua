...
local function check_removable(removable_type)
  return group_match(function (unit)
    local debuff, count, duration, expires, caster, found_debuff = UnitReverseDebuff(unit.unitID, bxhnz7tp5bge7wvu.data.removables[removable_type])
...
function group_under(...)
  ...
  return group_count(function (unit)
    return unit.alive
      and UnitInPartyShard(unit.unitID)
      and (
...
function bxhnz7tp5bge7wvu.environment.conditions.group()
  return setmetatable({ }, {
