...
function bxhnz7tp5bge7wvu.environment.conditions.powerType(unit, power_type, power_type_name)
  return setmetatable({
    unitID = unit.unitID,
    type = power_type,
    ...
    __index = function(t, k)
      if powerType[k] then
        ...
        bxhnz7tp5bge7wvu.console.debug(4, 'power', 'blue', t.unitID .. '.power.' .. t.type .. '.' .. k .. ' = ' .. bxhnz7tp5bge7wvu.format(result))
        return result
      end
    end,
    __unm = function(t)
        local result = powerType['actual'](t)
        bxhnz7tp5bge7wvu.console.debug(4, 'power', 'blue', t.unitID .. '.power.' .. t.type_name .. '.actual' .. ' = ' .. bxhnz7tp5bge7wvu.format(result))
        return result
    end
  })
end
